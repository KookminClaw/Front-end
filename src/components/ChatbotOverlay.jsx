...
export default function ChatbotOverlay({ open, onToggle }) {
  ...
  return (
    <>
      {open && (
        <div className="fixed bottom-[100px] right-8 w-[360px] h-[500px] bg-white rounded-[20px] shadow-[0_8px_40px_rgba(0,0,0,0.15)] z-[300] flex flex-col overflow-hidden">
          <div className="bg-primary px-5 py-4 flex items-center gap-2.5 text-white">
            <div className="w-9 h-9 bg-white/20 rounded-full flex items-center justify-center text-lg"><FaRobot /></div>
            <div>
      ...
      <div className="fixed bottom-8 right-8 w-14 h-14 bg-primary rounded-full flex items-center justify-center text-2xl cursor-pointer shadow-[0_4px_20px_rgba(79,110,247,0.4)] z-[300] transition-transform duration-200 text-white hover:scale-[1.08]" onClick={onToggle}>
        <IoChatbubblesSharp />
      </div>
    </>
  )
