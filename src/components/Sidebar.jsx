...
import { FaList,FaStar,FaCheck,FaGraduationCap,FaDollarSign,FaBriefcase,FaUser } from "react-icons/fa";
import { MdFlightTakeoff,MdSettings,MdEdit } from "react-icons/md";
import { BiSolidParty } from "react-icons/bi";

const navItem = 'flex items-center gap-2.5 px-3 py-[9px] rounded-lg cursor-pointer text-sm transition-all duration-150'
  ...
  return (
    <Link to={to}>
      <div className={isActive ? navItemActive : navItemInactive}>
        ...
        {children}
        {badge && <span className="ml-auto bg-accent text-white text-[10px] font-bold px-1.5 py-px rounded-full">{badge}</span>}
      </div>
    </Link>
  )
}
  ...
  return (
    <aside className="w-60 bg-white border-r border-line flex flex-col fixed top-0 left-0 bottom-0 z-[100]">
      <div className="px-6 py-5 border-b border-line flex items-center gap-2.5">
        <div className="w-[34px] h-[34px] bg-primary rounded-[10px] flex items-center justify-center text-white text-base"><FaGraduationCap /></div>
        <div>
      ...
      <div className="px-5 py-4 border-t border-line flex items-center gap-2.5 cursor-pointer">
        <div className="w-9 h-9 bg-primary-light rounded-full flex items-center justify-center text-base"><FaUser /></div>
        <div>
          <div className="text-[13px] font-semibold">소프트웨어학부</div>
          <div className="text-[11px] text-subtext">3학년</div>
        </div>
        <div className="ml-auto text-sm text-subtext"><MdEdit /></div>
      </div>
    </aside>
  )
}